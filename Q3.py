...
reward3 = {
    ...
    's2': {
        'a1': {'s2': 0, 's3': 5.0, 's1': 0},
        'a2': {'s7': 0, 's3': 5.0, 's1': 0, 's2': 0},
        'a3': {'s3': 5.0, 's2': 0, 's7': 0},
        'a4': {'s1': 0, 's2': 0, 's7': 0}
    },
    's3': {
        'a1': {'s3': 5.0, 's4': 0, 's2': 0},
        'a2': {'s8': 0, 's4': 0, 's2': 0, 's3': 5.0},
        'a3': {'s4': 0, 's3': 5.0, 's8': 0},
        'a4': {'s2': 0, 's3': 5.0, 's8': 0}
    },
    's4': {
        'a1': {'s4': 0, 's5': 0, 's3': 5.0},
        'a2': {'s9': 0, 's5': 0, 's3': 5.0, 's4': 0},
        'a3': {'s5': 0, 's4': 0, 's9': 0},
        'a4': {'s3': 5.0, 's4': 0, 's9': 0}
    },
    's5': {
...
def value_iteration3(gamma, value3, reward3, transition3, states, actions, policy3):
    for i in range(10000):
        ...
        for state in states:
            ...
            initial_v = value3[state]
            max_action = None
            max_value = float('-inf')

            for action in transition3[state]:
                action_value = 0.0
                for next_state in transition3[state][action]:
                    action_value += transition3[state][action][next_state] * (reward3[state][action][next_state] + gamma * value3[next_state])

                if action_value > max_value:
                    max_value = action_value
                    max_action = action

            value3[state] = max_value
            policy3[state] = max_action
            # print('state after', value1[state], 'state', state)
            
            delta = max(delta, abs(initial_v - value3[state]))
        
        if delta < 0.0001:
            print('Iteration number:', i+1)
            print('\n')
            break
    return policy3, value3

policy3, value3 = value_iteration3(gamma, value3, reward3, transition3, states, actions, policy3)

rounded_value3 = {key: round(value, 4) for key, value in value3.items()}

# print(policy1)
# print(rounded_value3)
...
for i in range(0, 25, 5):
    ...
    for key in row:
        if key not in rounded_value3:
            print('0.0000', end='\t')
        else:
            print(f"{rounded_value3[key]:.4f}", end='\t')
    print()
print('\n')    
print('Policy')
