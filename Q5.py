...
reward4 = {
    ...
    's2': {
        'a1': {'s2': 0, 's3': 4.484, 's1': 0},
        'a2': {'s7': 0, 's3': 4.484, 's1': 0, 's2': 0},
        'a3': {'s3': 4.484, 's2': 0, 's7': 0},
        'a4': {'s1': 0, 's2': 0, 's7': 0}
    },
    's3': {
        'a1': {'s3': 0},
    ...
    's4': {
        'a1': {'s4': 0, 's5': 0, 's3': 4.484},
        'a2': {'s9': 0, 's5': 0, 's3': 4.484, 's4': 0},
        'a3': {'s5': 0, 's4': 0, 's9': 0},
        'a4': {'s3': 4.484, 's4': 0, 's9': 0}
    },
    's5': {
        'a1': {'s5': 0, 's4': 0},
        'a2': {'s10': 0, 's5': 0, 's4': 0},
    ...
    's8': {
        'a1': {'s3': 4.484, 's9': 0, 's7': 0, 's8': 0},
        'a2': {'s8': 0, 's9': 0, 's7': 0},
        'a3': {'s9': 0, 's3': 4.484, 's8': 0},
        'a4': {'s7': 0, 's3': 4.484, 's8': 0}
    },
    's9': {
...
def value_iteration4(gamma, value4, reward4, transition4, states, actions, policy4):
    for i in range(10000):
        ...
        for state in states:
            ...
            initial_v = value4[state]
            max_action = None
            max_value = float('-inf')

            for action in transition4[state]:
                action_value = 0.0
                for next_state in transition4[state][action]:
                    action_value += transition4[state][action][next_state] * (reward4[state][action][next_state] + gamma * value4[next_state])

                if action_value > max_value:
                    max_value = action_value
                    max_action = action

            value4[state] = max_value
            policy4[state] = max_action
            # print('state after', value1[state], 'state', state)
            
            delta = max(delta, abs(initial_v - value4[state]))
        
        if delta < 0.0001:
            print('Iteration number:', i+1)
            print('\n')
            break
    return policy4, value4

policy4, value4 = value_iteration4(gamma, value4, reward4, transition4, states, actions, policy4)

rounded_value4 = {key: round(value, 4) for key, value in value4.items()}
...
for i in range(0, 25, 5):
    ...
    for key in row:
        if key not in rounded_value4:
            print('0.0000', end='\t')
        else:
            print(f"{rounded_value4[key]:.4f}", end='\t')
    print()
print('\n')    
print('Policy')
