...
def value_iteration1(gamma, value1, reward, transition, states, actions, policy1):
        ...
        for state in states:
            ...
            for action in transition[state]:
                ...
                for next_state in transition[state][action]:
                    action_value += transition[state][action][next_state] * (reward[state][action][next_state] + gamma * value1[next_state])

                if action_value > max_value:
                    max_value = action_value
                    max_action = action

            value1[state] = max_value
            policy1[state] = max_action
            # print('state after', value1[state], 'state', state)
            
            delta = max(delta, abs(initial_v - value1[state]))
        
        if delta < 0.0001:
            print('Iteration number:', i+1)
            print('\n')
            break
    return policy1, value1

policy1, value1 = value_iteration1(gamma, value1, reward, transition, states, actions, policy1)

rounded_value1 = {key: round(value, 4) for key, value in value1.items()}
...
for i in range(0, 25, 5):
    ...
    for key in row:
        if key not in rounded_value1:
            print('0.0000', end='\t')
        else:
            print(f"{rounded_value1[key]:.4f}", end='\t')
    print()
print('\n')   
print('Policy')
